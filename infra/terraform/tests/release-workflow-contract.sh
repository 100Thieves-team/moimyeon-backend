...
assert_contains "${ROLLBACK_WORKFLOW}" 'LIVE_ROLLBACK_ENABLED.*MOIMYEON_LIVE_ROLLBACK_ENABLED' "live rollback은 별도 명시적 flag가 필요하다."
assert_contains "${ROLLBACK_WORKFLOW}" 'Live rollback is disabled' "live rollback false 상태는 mutation 전에 실패해야 한다."
assert_contains "${ROLLBACK_WORKFLOW}" 'Live rollback is disabled before AWS credential use' "live rollback job rerun도 AWS credential 전에 flag를 재검증해야 한다."
...
assert_contains "${PROMOTE_WORKFLOW}" 'MOIMYEON_TERRAFORM_LIVE_CI_ENABLED' "live promotion은 live Terraform 활성 상태도 재검증해야 한다."
assert_contains "${PROMOTE_WORKFLOW}" 'Live promotion is disabled before AWS credential use' "live flag race는 AWS credential 전에 fail-closed해야 한다."
