...
assert_contains "${SYNC_SCRIPT}" 'phase environments' "AWS role보다 GitHub Environment 보호 규칙을 먼저 만들어야 한다."
assert_contains "${SYNC_SCRIPT}" 'phase variables' "AWS apply 뒤 output Variables를 별도 단계로 동기화해야 한다."
assert_contains "${SYNC_SCRIPT}" 'OIDC bootstrap ordering' "두 bootstrap phase를 한 mutation으로 합치면 안 된다."
...
assert_contains "${SYNC_SCRIPT}" 'configure_environment terraform-review-plan true' "PR plan은 reviewer gate가 필요하다."
assert_contains "${SYNC_SCRIPT}" 'configure_environment shared-infra true' "shared apply는 reviewer gate가 필요하다."
assert_contains "${SYNC_SCRIPT}" 'configure_environment dev-infra true' "dev apply는 reviewer gate가 필요하다."
assert_contains "${SYNC_SCRIPT}" 'configure_environment live-infra true' "live apply는 reviewer gate가 필요하다."
assert_contains "${SYNC_SCRIPT}" 'configure_environment live-infra true "dev"' "workflow_run 기반 live apply Environment는 default branch ref를 허용해야 한다."
