...
이번 라운드는 3스킬 × 12프롬프트 × (claude 2회 + codex 1회) = **108개
콜드 세션**을 한 번에 돌렸다(실제 완주분은 한도로 그보다 적다) — 이전 라운드(스킬당 36세션)의 3배 배치.
...
## 모델 변동성 확인 — opus 라운드 (2026-08-25)

같은 27건을 `--model claude-opus-5`로 재실행해 모델이 트리거 판정의 교란
변수인지 확인했다. 원본: `trigger-routing-opus-*.csv`.
...
**집계가 동일하다.** 개별 불일치는 2건뿐이고 둘 다 ship-pr 내부에서
p1↔p2가 뒤바뀐 것(opus는 p2 호출·p1 미호출, sonnet은 그 반대)이다.

### ship-pr 미호출의 진짜 원인 — 측정 환경 결함

두 라운드 모두 **변경분이 없는 clean 워크트리**에서 측정했다. ship-pr은
1단계가 "변경분 검토"라 커밋할 것이 없으면 스킬 진입 대신 상태 보고로
빠진다. 워크트리에 실제 변경을 만들고 재측정한 결과:

| | opus | sonnet |
| --- | --- | --- |
| p1 "커밋하고 PR 올려줘" | yes | yes |
| p2 "이 변경분 커밋 나눠서 PR 만들어줘" | yes | yes |

**4/4 호출.** 즉 clean 워크트리의 미호출은 트리거 결함이 아니라 측정
조건의 결함이었고, 정상 조건에서 **양성은 실질 18/18**이다.

### 결론

- 모델은 트리거 판정의 교란 변수로서 미미하다 — 이전 라운드들(모델 미기록)
  과의 비교도 이 범위에서는 유효하다고 본다.
- 라우팅 표 등재 + 스킬 9종 경쟁 상태에서 **양성 실질 100%, 오호출 0%**.

## 추가 교훈 (모델 변동성 라운드)

12. **스킬의 전제 조건을 만족시키지 않은 채 트리거를 재면 미호출이 나온다.**
    ship-pr처럼 "변경분이 있어야" 의미가 있는 스킬은 워크트리를 dirty로
    만들고 재야 한다. 트리거 세트에 전제 조건을 명시할 것.
