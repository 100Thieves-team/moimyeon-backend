...
현재 구현과 사용 방법은 [로깅 기반 작업 보고서](../../docs/architecture/logging-report.md)를 먼저 읽는다. 아래는 2026-09-14의 전체 목표 설계다. 이후 공통 설정·HTTP 수집·S3 인프라 코드를 구현했지만 실제 AWS 전환과 알림·그로스 연결은 남아 있다. 단계별 결과는 [구현 기록](implementation.md)을 따른다.
