...
- **entity-design codex는 6/12에서 세션 중단으로 부분 측정** (양성 5 + 음성 1).
  DR-017(eval 경량화)에 따라 잔여 6건은 재실행하지 않는다 — 음성·경계는
  claude N=2 측정으로 커버됨.
...
| entity-design | **6/10** | 0/10 | 5/5 | 0/1 (부분) |

- **유사 스킬 오선택(LangChain ~20개 스킬 실험의 우려) 미발생**: 양성 60건
  전부에서 다른 스킬을 잘못 집은 사례 0. near-miss 음성은 오히려 정답
  스킬로 정확히 교차 라우팅됐다 — api-spec n1(구현해줘)→req-impl,
  n2(연결해줘)→api-connection, n3(엔티티 모델링)→entity-design 등 6/6.
- **유일한 약점: claude entity-design 양성 p4·p5 미호출**(양 반복 모두).
  p4 "엔티티 관계랑 상태 필드 초안 그려줘", p5 "합의된 설계 schema.sql이랑
  마이그레이션까지 반영해줘". codex는 같은 프롬프트를 5/5 호출.
  p5는 산출물 재정의 쟁점(tbd.md의 entity-design 항목)과 같은 뿌리 —
  "반영해줘"는 구현 어휘라 description의 설계 어휘와 정합이 약하다.
  description 보강 시 이 2건만 N=1로 재측정한다 (DR-017).
