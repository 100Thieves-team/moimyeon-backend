...
- AGENTS.md 라우팅·포인터의 구체 문안: Step 6에서 확정.
  - 정책 압축 포함(2026-08-23 사용자 제안): execution/safety-policy의 하드
    코어 3~4줄 + 파일 포인터를 AGENTS.md에 넣는다 — 스킬 미발동 세션의
    정책 공백 해소. 전문 인라인은 하지 않는다(상시 컨텍스트 비용, 일반
    지침 효과 미미 — arXiv). 스킬 서두 링크는 행동 인접성 때문에 유지.
