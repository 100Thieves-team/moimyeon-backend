...
- `entity-design`을 독립 스킬로 유지할지: 절차가 짧아 스킬 가치가 불확실.
  ...
  - 베이스라인 실측(2026-08-23): claude 양성 6/10 — p4("상태 필드 초안
    그려줘")·p5("schema.sql이랑 마이그레이션까지 반영해줘") 미호출.
    5스킬 중 유일한 트리거 약점. p5의 "반영" 어휘는 아래 산출물 재정의
    쟁점과 같은 뿌리. description 보강 시 실패 2건만 N=1 재측정(DR-017).
