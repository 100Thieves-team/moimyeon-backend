...
class ApiResponseAuthErrorWriter(
    ...
    override fun writeUnauthorized(response: HttpServletResponse) = write(response, ErrorType.AUTHENTICATION_REQUIRED)

    override fun writeForbidden(response: HttpServletResponse) = write(response, ErrorType.ACCESS_DENIED)

    private fun write(response: HttpServletResponse, errorType: ErrorType) {
