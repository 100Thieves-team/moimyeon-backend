...
class CompanyFinder(
    ...
    fun search(query: String): List<Company> {
        log.debug { "company.finder.search" }
        return searchByPrefixes(listOf(CompanyNameNormalizer.normalize(query))).companies
    }
    ...
    fun searchByPrefixes(candidates: List<String>): CompanyNameMatch {
        log.debug { "company.finder.searchByPrefixes candidatesCount=${candidates.size}" }
    ...
    fun getByIds(companyIds: Collection<Long>): List<Company> {
        log.debug { "company.finder.getByIds companyIdsCount=${companyIds.size}" }
