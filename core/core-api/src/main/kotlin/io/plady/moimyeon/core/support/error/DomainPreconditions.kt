 ...
 * [ErrorType]에 매핑된 정상 응답(4xx 등)으로 처리되게 한다.
 *
 * 구분 기준:
 ...
 *
 * [ErrorType]이 메시지를 이미 내장하므로 표준 require 같은 메시지 람다는 두지 않는다.
 * 동적 상세는 [data]로 실어 응답/로그에 전달한다. `contract` 덕에 호출 이후 스마트 캐스트가 동작한다.
 */
@OptIn(ExperimentalContracts::class)
fun requireBusiness(condition: Boolean, errorType: ErrorType, data: Any? = null) {
...
@OptIn(ExperimentalContracts::class)
fun <T : Any> requireFound(value: T?, errorType: ErrorType, data: Any? = null): T {
