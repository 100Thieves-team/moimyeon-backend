...
class ProfileServiceIT(
    ...
    fun `가입(자동 동의)부터 소개 저장·완성 상태 전환까지의 흐름이 동작한다`() {
        ...
        // when — 관심 직무 1(서버·백엔드), sigunguId=1(강남구) — seed.sql 참조 데이터
        val createdId = profileService.create(
            memberId,
            ProfileContent(bio = null, meetingPreference = null, sigunguId = 1L, interestJobRoleIds = listOf(1L)),
        )
        ...
        assertThat(found.interestJobRoleIds).containsExactly(1L)
    ...
    fun `프로필 수정은 전체 교체와 관심 직무·회사 교체를 지원한다`() {
        ...
        profileService.create(memberId, ProfileContent(bio = null, meetingPreference = null, sigunguId = null))

        // when — 참조 id 는 seed.sql: jobRole 1(서버·백엔드)·2(프론트엔드), sigunguId 2(마포구), company 1·2·3
    ...
    fun `같은 회원은 프로필을 다시 작성할 수 없다`() {
        ...
        assertThatThrownBy {
            profileService.create(
                memberId,
                ProfileContent(bio = null, meetingPreference = null, sigunguId = null, interestJobRoleIds = listOf(1L)),
            )
