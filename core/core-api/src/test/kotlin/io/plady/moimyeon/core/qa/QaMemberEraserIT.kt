...
class QaMemberEraserIT(
    ...
    fun `QA 생성 회원을 남긴 행과 소유 행까지 지우고 다른 회원은 손대지 않는다`() {
        ...
        assertThat(deleted.participants).isEqualTo(2)
        assertThat(deleted.attendances).isEqualTo(1)
        assertThat(deleted.questions).isEqualTo(3)
        assertThat(questionRepository.findAll().filter { it.roomId == others }).isEmpty()
        assertThat(deleted.reviews).isEqualTo(1)
        assertThat(deleted.reviewTags).isEqualTo(1)
        ...
        assertThat(participationRepository.findAll().filter { it.roomId == others }).hasSize(1)
