...
class ResumeManagementIT(
    ...
    fun `보관 목록은 기본 이력서를 먼저 보여주고 나머지는 최신순으로 보여준다`() {
        ...
        val defaultResumeId = resumeRegistrar.register(memberId, newResume(memberId, "default.pdf"))
        val olderResumeId = resumeRegistrar.register(memberId, newResume(memberId, "older.pdf"))
        val newerResumeId = resumeRegistrar.register(memberId, newResume(memberId, "newer.pdf"))
    ...
    fun `기본 이력서 변경은 기존 기본 해제와 새 기본 지정을 함께 커밋한다`() {
        ...
        val firstResumeId = resumeRegistrar.register(memberId, newResume(memberId, "first-default.pdf"))
        val secondResumeId = resumeRegistrar.register(memberId, newResume(memberId, "second-default.pdf"))
    ...
    fun `기본 이력서를 삭제하면 남은 이력서 중 최신 이력서를 기본으로 지정한다`() {
        ...
        val defaultResumeId = resumeRegistrar.register(memberId, newResume(memberId, "default.pdf"))
        val olderResumeId = resumeRegistrar.register(memberId, newResume(memberId, "older.pdf"))
        val latestResumeId = resumeRegistrar.register(memberId, newResume(memberId, "latest.pdf"))
    ...
    fun `유일한 기본 이력서를 삭제하면 보관함이 비게 된다`() {
        ...
        val resumeId = resumeRegistrar.register(memberId, newResume(memberId, "only.pdf"))
    ...
    fun `같은 이력서 삭제를 반복해도 삭제 상태를 유지한다`() {
        ...
        resumeRegistrar.register(memberId, newResume(memberId, "default-idempotent-delete.pdf"))
        val resumeId = resumeRegistrar.register(memberId, newResume(memberId, "idempotent-delete.pdf"))
    ...
    fun `이력서 이름을 변경해도 원본 파일 정보는 유지한다`() {
        ...
        val resumeId = resumeRegistrar.register(memberId, newResume(memberId, "backend.pdf"))
    ...
    fun `삭제한 이력서는 이름을 변경할 수 없다`() {
        ...
        resumeRegistrar.register(memberId, newResume(memberId, "default-deleted-rename.pdf"))
        val deletedResumeId = resumeRegistrar.register(memberId, newResume(memberId, "deleted-rename.pdf"))
