...
class DevAuthProfileContextTest {
    private val contextRunner = ApplicationContextRunner()
        ...
        .withUserConfiguration(DevAccessTokenIssuer::class.java, DevAuthController::class.java)
    ...
    fun `local과 local-dev와 dev 프로파일에서 개발 인증 엔드포인트를 등록한다`() {
        listOf("local", "local-dev", "dev").forEach { profile ->
            contextRunner.withPropertyValues("spring.profiles.active=$profile").run { context ->
                assertThat(context).hasSingleBean(DevAccessTokenIssuer::class.java)
    ...
    @Test
    fun `staging 또는 live 프로파일이 함께 활성화되면 개발 인증 엔드포인트를 등록하지 않는다`() {
        listOf("dev,staging", "dev,live").forEach { profiles ->
            contextRunner.withPropertyValues("spring.profiles.active=$profiles").run { context ->
                assertThat(context).doesNotHaveBean(DevAccessTokenIssuer::class.java)
