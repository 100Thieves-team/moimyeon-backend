...
class ProfileControllerTest : RestDocsTest() {
    ...
    private val createProfileDescription =
        "온보딩에서 소개 정보를 받아 프로필을 생성하고 프로필 완성 상태로 전환한다. 모든 필드는 선택 입력이다" +
            ...
            "관심 직무·지역은 카탈로그 참조 id 를 받는다(/v1/job-roles, /v1/regions). 관심 직무는 다건이다. " +
    ...
    private val updateProfileDescription =
        "마이페이지 수정 폼의 전체 교체 저장. 관심 직무·지역·관심 회사는 카탈로그 참조 id 를 받는다. " +
                ...
                documentApi(
                    ...
                    requestFields(
                        fieldWithPath("interestJobRoleIds").type(JsonFieldType.ARRAY).optional()
                            .description("관심 직무 id 목록 (선택, /v1/job-roles — 전체 교체)"),
