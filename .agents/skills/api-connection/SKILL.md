...
1. **전제·컨텍스트 확인** — 대상 엔드포인트의 Controller(모킹)와 구현된
   ...
   워크플로우가 필요한지 알리고 정지한다. `.worklog/{작업키}/context.md`와
...
3. **연결 계획** — `.worklog/{작업키}/plan.md`: 어느 Service를 어느
...
5. **리뷰** — `.agents/agents/code-reviewer.md` 위임 (변경 파일 +
   review-diff.patch + context.md 경로 — 위임 전 diff를 `.worklog/{작업키}/review-diff.patch`로 저장해 함께 준다.)
   반영 상한 2회 —
