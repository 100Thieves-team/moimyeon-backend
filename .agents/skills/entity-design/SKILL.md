...
3. **논리 모델 작성** — `.worklog/{작업키}/erd.dbml`에 DBML로: 엔티티·관계,
   핵심 상태 필드와 전이, 유니크·존재 종속 제약. 타입은 논리 수준까지만 —
   MySQL 타입·인덱스 확정은 2단 물리 모델링의 몫이다. **미결정 목록**(정책
   판단이 필요한 것)은 `.worklog/{작업키}/tbd.md`에 — 추측으로 채우지
...
7. **리뷰** — `.agents/agents/db-reviewer.md` 위임 (마이그레이션·엔티티·
   schema.sql 등 변경된 스키마 산출물 전부 + review-diff.patch + context.md 경로 — 위임 전 diff를 `.worklog/{작업키}/review-diff.patch`로 저장해 함께 준다.) ddl_analysis 판정 포함. 반영 상한 2회 — 소진하면
