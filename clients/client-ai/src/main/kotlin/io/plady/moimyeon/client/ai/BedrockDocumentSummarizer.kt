...
import io.plady.moimyeon.core.domain.resume.DocumentSummarizer
...
@Component
class BedrockDocumentSummarizer(
    chatClientBuilder: ChatClient.Builder,
) : DocumentSummarizer {
    private val chatClient = chatClientBuilder.build()

    override fun summarizePdf(content: ByteArray): String {
        val response = try {
            chatClient.prompt()
            ...
            throw DocumentSummarizationException(exception)
        ...
        if (response.isNullOrBlank()) {
            throw DocumentSummarizationException()
        }
        return response
    }
