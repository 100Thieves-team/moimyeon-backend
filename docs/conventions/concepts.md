...
```text
core.domain
└── {개념}            개념 명사 패키지. 예: order
    ├── {도메인 모델}   예: Order, OrderNumber(VO)
    ├── {개념}Service   예: OrderService
    └── {Implement}     예: OrderFinder, OrderManager, OrderMapper, OrderNumberGenerator
```

어느 패키지에 둘지 애매하면 "이 행위·상태가 누구의 불변식과 함께 움직이는가"로 판단한다.
두 개념을 잇는 부수 개념(연결 이력 등)은 판정 책임을 가진 쪽 패키지에 둔다
(예: 쿠폰 사용 이력은 coupon 패키지 — "이 쿠폰을 쓸 수 있나?" 판정을 coupon 쪽이 소유).
...
- **id 참조가 기본**: 다른 개념을 알아야 하면 도메인 객체가 아니라 **식별자만** 든다.
  예: 주문은 계정의 id 만 안다(`Order.accountId: UUID`). 역방향(계정이 주문의 존재를
  아는 것)은 필요해질 때까지 만들지 않는다. 요청 DTO의 `toXxx()` 결과에도 사용자가 선택한
  `productIds` 같은 참조 식별자는 포함할 수 있다. API 변환에서 분리하는 식별자는 요청 값이 아닌
  `currentMember.id` 같은 인증 주체 식별자다([api-design.md](api-design.md)).
- **파생 가능한 사실은 저장하지 않는다**: 다른 데이터에서 계산되는 값을 중복 저장하면 정합성
  관리 대상만 늘어난다. 예: 주문의 "총 상품 수"는 컬럼이 아니라 주문라인 행을 세는 계산이다.
- **다른 개념의 규칙은 그 개념의 도구에게 묻는다**: 여러 개념이 얽힌 규칙은 상대 개념의 내부를
  뒤지지 않고, 상대가 노출한 판정(`ProductRefValidator.validateProducts` 등)을 쓴다.
  판정과 예외는 그 개념이 함께 소유한다. 물어보는 쪽이 `Boolean` 을 받아 자기가 에러로 바꾸면
  같은 매핑이 호출자마다 복제된다.
- **성격이 같으면 한 격벽**: 데이터가 여럿이라도 소유자·수명·접근 방식이 같으면 하나의 개념으로
  묶는다(예: 배치가 채우는 읽기 전용 참조 데이터들 → `catalog` 하나). 성격이 갈라지면 그때 쪼갠다.
  회사처럼 앱 등록·운영 검증·병합이라는 별도 생명주기가 생기면 읽기 전용 `catalog`에서 분리한다.
...
- **개념 객체가 다른 개념을 이미 알고 있으면 Service 가 그 개념의 Implement 를 직접 써도 된다.**
  ...
  주입받는다.
  반대로 **`Account` 가 주문의 존재를 모르면** `AccountService` 는 `OrderFinder` 를 쓰지 않는다 —
  둘을 함께 보여줘야 하는 응답은 Facade 가 조합한다.
...
- **흐름 분기를 Implement 에 숨기지 않는다.** `findOrRegister` 류의 합성 메서드 금지 —
  ...
  (예: 외부 신원으로 로그인할 때 기존 계정이면 로그인, 아니면 가입 — 이 분기는 Service 에 남는다).
  단 **한 커밋 단위의 조합은 Implement 가 소유한다** — 두 개념의 쓰기가 함께 커밋돼야 하면
  그 조합 자체가 하나의 개념이다(`OrderPlacer` — 주문 생성 + 재고 차감).
  판별 기준(그 단계들 사이에 외부 I/O 가 끼는가)은 [layers.md](layers.md)의
  "Implement 도 조합한다" 절.
- 상태로 개념을 오염시키지 않는다. 예: "탈퇴한 계정"은 도메인 세계의 `Account` 개념에 들어오지 못하게
  **persistence 경계(Finder)에서 걸러낸다**. 도메인 객체는 상태 분기를 모른다 ([layers.md](layers.md)).
- 개념 규칙은 값 객체가 보증한다. 예: 주문번호 형식·길이는 `OrderNumber` VO 생성 시점에 검증된다.
...
- **도메인 객체의 필드는 nullable 하지 않는 것을 지향한다.** null 은 업무적으로 정말 "없을 수 있는"
  ...
  (예: 배송지 변경이면 변경에 필요한 값만 가진 행위 객체)를 만들거나 전체를 조회해 갱신하는 쪽으로
