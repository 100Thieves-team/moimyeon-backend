...
사고·리뷰에서 배운 것은 날짜와 함께 한 줄씩 추가한다
(형식: README의 "어떻게 늘리는가").
- 2026-09-03: Bedrock 모델이 카탈로그에 ACTIVE여도 계정 agreement가 `NOT_AVAILABLE`이면 모든 호출이 403으로 실패한다.
  배포 전 `GetFoundationModelAvailability`의 authorization·entitlement·region·agreement를 모두 확인하고,
  개인정보 문서는 Global CRIS가 아니라 요구 지역의 direct model ID를 사용한다.
- 2026-09-03: 평가 표현을 금지한 프롬프트도 극단 장문 입력에서는 `광범위한 경험`, `~로 보입니다`를 출력했다.
  법적·제품적 출력 경계는 프롬프트에만 맡기지 말고 결정적으로 검사하며, 제한된 재생성 후에도 위반하면 실패시킨다.
- 2026-09-03: 동기 LLM 재생성은 호출 timeout의 곱이 ALB·도메인 상태 만료를 넘을 수 있다.
  호출별 timeout뿐 아니라 전체 처리 예산을 두고, 다음 호출을 마칠 시간이 남을 때만 재생성을 시작한다.
