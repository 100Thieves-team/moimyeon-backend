...
- 2026-08-27: 개발 인증 빈의 프로파일 식이 `dev,staging` 혼합 활성화에서 staging 등록을 허용했다.
  원인: 개발 프로파일 허용식에 `!live`만 두고 `!staging`을 빠뜨렸다. 재발 방지: 운영성 프로파일을
  모두 명시적으로 제외하고 혼합 프로파일 컨텍스트 테스트로 고정한다.
