...
- 2026-08-25: 게이트의 음성 테스트를 **합성 픽스처로만** 짰더니 실제
  코드베이스에서만 나는 오탐 2건을 못 잡았다. 검출기·검증기를 만들 때는
  실레포 전수 스캔을 회귀 테스트에 넣는다 (MOI-474 DR-030).
- 2026-08-25: 오탐을 줄이는 수정이 미탐을 늘렸다. **두 번 같은 방식으로**
  — 값의 *형태*로 예외를 줬더니 그 형태의 진짜 시크릿이 통과했고(DR-030),
  줄 *어딘가의* `${` 존재로 예외를 줬더니 시크릿 뒤에 `${VAR}`만 붙이면
  통과했다(DR-033). 규칙: **검출기의 예외는 매치된 그 자리를 근거로만
  준다** — 매치 위치, 매치가 캡처한 값. 줄 전체나 값의 생김새는 근거가
  아니다. 오탐 1건을 없애려는 수정일수록 우회 케이스를 먼저 쓴다.
- 2026-09-11: CI job 추가가 checkout 개수 2개를 가정한 self-test를 깨뜨렸다(MOI-516).
  개수가 아닌 각 step의 SHA 고정을 검증하고, PR 전 개별 게이트와 gate self-test 전체를 함께 실행한다.
