...
CREATE TABLE room_status_log (
    ...
    handler_type      VARCHAR(20) NOT NULL DEFAULT 'MEMBER',
...
-- 출석 기록. 기본 흐름은 진행 확정 시 확정 참여자 전원을 출석으로 저장한다.
-- 레거시 진행 시작 흐름에서는 확정 참여자별 출석·불참 선택을 한 번 저장하며 일반 사용자는 수정하지 않는다.
-- 라운드 배정과 무관하고, 레거시 흐름에서 시작 후 도착해도 최초에 기록된 불참 상태를 유지한다.
